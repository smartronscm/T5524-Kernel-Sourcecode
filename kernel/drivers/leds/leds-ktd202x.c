...
static int ktd202x_suspend(struct device *dev)
{
    ...
    if (!(chip->brightness_red || chip->brightness_green || chip->brightness_blue)) {
        if (the_ktd202x_chip->power_status) {
            i2c_smbus_write_byte_data(chip->client, EN_RESET_CONTROL, 0x08);//Device sleep.
...
static int ktd202x_resume(struct device *dev)
{
    ...
    if (!(chip->brightness_red || chip->brightness_green || chip->brightness_blue)) {
        if (!the_ktd202x_chip->power_status) {
            i2c_smbus_write_byte_data(chip->client, EN_RESET_CONTROL, 0x7);//Device restart.
...
void led_all_on_test(struct ktd202x_chip *chip)
{
	i2c_smbus_write_byte_data(chip->client, 0x06, 0x20);
	i2c_smbus_write_byte_data(chip->client, 0x07, 0x20);
	i2c_smbus_write_byte_data(chip->client, 0x08, 0x20);
	i2c_smbus_write_byte_data(chip->client, LED_ENABLE_CONTROL, 0x15);// initialization LED off
}
...
static int ktd202x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    chip->color = 0;
	i2c_smbus_write_byte_data(chip->client, EN_RESET_CONTROL, 0x7);//reset
	my_delay(10000);
	i2c_smbus_write_byte_data(chip->client, LED_ENABLE_CONTROL, 0x00);
