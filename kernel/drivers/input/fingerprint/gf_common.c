...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
	#if 1 //reset irq value can be 0x100, 400 0r 800, as different chip type.
		...
		while (count > 0) {
		    ...
		    //if(0x800 == (irq_value[0]<<8 | irq_value[1])) {
...
static int gf_probe(struct spi_device *spi)
{
	...
	int retval = 0;
	int status = -EINVAL;
	printk("gf probe begin. add by xipeng.gu\n");
