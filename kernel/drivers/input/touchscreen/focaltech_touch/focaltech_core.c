...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (!rc)
    {
        ...
        printk(KERN_ALERT"max_touch_number=%d", pdata->max_touch_number);
    }
    else
    {
...
static int fts_ts_resume(struct device *dev)
{
    ...
    fts_release_all_finger();
        ...
        schedule_delayed_work(&data->delay_resume_work, msecs_to_jiffies(200));
...
static void ft5x0x_ts_delay_resume_work(struct work_struct *work)
{
        struct fts_ts_data *data = container_of((struct delayed_work *)work, struct fts_ts_data, delay_resume_work);
